...
| fieldsAdd error_rate = arraySum(errors) * 100.0 / arraySum(actions),
  ...
  total_actions = arraySum(actions),
  total_errors = arraySum(errors)
| fields frontend.name, error_rate, avg_duration_ms, total_actions, total_errors
...
| limit 50`
  ...
  const rows: ScorecardRow[] = (scorecard.records ?? []).map((r) => {
    ...
    return {
      ...
      status: getStatus(errRate, dur),
      metric1: `${errRate.toFixed(1)}%`,
      metric2: `${Math.round(dur)} ms`,
  ...
  return (
    <Flex flexDirection="column" gap={24} style={{ padding: '24px' }}>
      <Flex flexDirection="column" gap={4}>
        ...
        <Text>Performance and availability for all monitored web applications.</Text>
      </Flex>
